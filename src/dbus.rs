...
struct DbusContext {
    ...
    reset_rtc_mode_timer_id: Arc<AtomicUsize>,
    ...
    cr.register(INTERFACE_NAME, |b: &mut IfaceBuilder<DbusContext>| {
        ...
        //b.method(
            ...
            //move |_, context, (active_raw,): (u8,)| {
                //info!("from  dbus RTC audio activity");
        ...
        b.method(
            ...
            ("mode", "timeout_sec"),
            (),
             |_, context, (active_raw, timeout_raw): (u8, u32)| {
                 info!("from  dbus RTC audio activity");
                let active = common::RTCAudioActive::try_from(active_raw)
                    .map_err(|_| MethodErr::failed("Unsupported RTCAudioActive"))?;
                //match common::set_rtc_audio_active(
                    //context.power_preferences_manager.as_ref(),
                    //active,
                //) {
                    //Ok(_) => {
                        //info!("success to set RTC audio activity");
                        //}
                    //Err(_) => {
                        //info!("Failed to set RTC audio activity");
                    //}
                //}
                let timeout = Duration::from_secs(timeout_raw.into());
                common::set_rtc_audio_active(context.power_preferences_manager.as_ref(), active)
                    .map_err(|e| {
                        error!("set RTC failed: {:#}", e);

                        MethodErr::failed("Failed to set rtc mode")
                    })?;

                context
                    .reset_rtc_mode_timer_id
                    .fetch_add(1, Ordering::Relaxed);
                let timer_id = context
                    .reset_rtc_mode_timer_id
                    .load(Ordering::Relaxed);
                let power_preferences_manager = context.power_preferences_manager.clone();
                let reset_rtc_mode_timer_id =
                    context.reset_rtc_mode_timer_id.clone();
                tokio::spawn(async move {
                    tokio::time::sleep(timeout).await;
                    if timer_id == reset_rtc_mode_timer_id.load(Ordering::Relaxed)
                        && common::set_rtc_audio_active(
                            power_preferences_manager.as_ref(),
                            common::RTCAudioActive::Inactive,
                        )
                        .is_err()
                    {
                        info!("Reset set_rtc_audio_active failed.");
                    }
                });
        ...
        b.method(
            ...
            |_, context, (mode_raw, timeout_raw): (u8, u32)| {
                info!("from  dbus SetFullscreenVideoWithTimeout");
...
pub async fn service_main() -> Result<()> {
    ...
    let context = DbusContext {
        ...
        reset_rtc_mode_timer_id: Arc::new(AtomicUsize::new(0)),
	...
	info!("vaibhav: from service_main");
	//monitor_cpu_utilization_async().await;
